  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
      <div className="flex items-center justify-between">
        <div>
           <h1 className="text-3xl font-black text-text-primary dark:text-white tracking-widest uppercase">Privacy Workspace</h1>
           <p className="text-text-muted dark:text-slate-500 text-[11px] font-black uppercase tracking-[0.4em] mt-2">Security Tiers & Access Governance</p>
        </div>
        <div className="flex gap-3">
           <Button variant="outline" size="sm" className="h-14 bg-card-bg border-none shadow-soft font-black px-8 uppercase tracking-widest text-[10px]">Audit Manifest</Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Active Sessions', val: 12, icon: Activity, color: 'bg-success' },
          { label: 'Neural Protocols', val: 842, icon: Database, color: 'bg-indigo-600' },
          { label: 'Privileged Nodes', val: 56, icon: Shield, color: 'bg-blue-600' },
          { label: 'Integrity Rating', val: '99.9%', icon: Zap, color: 'bg-warning' },
        ].map(s => (
          <Card key={s.label} className="p-6 h-28 bg-card-bg border-none shadow-soft rounded-2xl flex items-center gap-6 group hover:translate-y-[-4px] transition-all">
            <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center text-white shrink-0 shadow-lg", s.color)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 p-0 overflow-hidden bg-card-bg border-none rounded-[3rem] shadow-soft flex flex-col">
           <div className="p-10 border-none flex justify-between items-center bg-bg-app">
              ...
              <div className="relative group">
                 ...
                 <input 
                   ...
                   className="bg-bg-app border-none rounded-xl pl-12 pr-6 h-10 text-[10px] font-black tracking-widest uppercase w-64 focus:ring-2 focus:ring-accent/5 outline-none transition-all text-text-primary placeholder:opacity-30"
                 />
              </div>
           </div>
              ...
              <table className="w-full text-left border-collapse">
                 <thead>
                    <tr className="bg-bg-app/50">
                       <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-text-muted text-left align-middle">Identity Cluster</th>
                       <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-text-muted text-left align-middle">Level</th>
                       <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-text-muted text-left align-middle">Status</th>
                       <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-text-muted text-left align-middle">Telemetry</th>
                    </tr>
                 </thead>
                      ...
                      <tr key={i} className="hover:bg-bg-app/50 transition-all duration-300">
                         <td className="px-10 py-8">
                            <div className="flex items-center gap-5">
                               <div className="w-11 h-11 rounded-xl bg-bg-app flex items-center justify-center font-black text-accent text-[12px]">{a.user.split(' ').map(n=>n[0]).join('')}</div>
                               <div>
        ...
        <div className="space-y-8">
           <Card className="p-10 border-none bg-card-bg rounded-[3rem] shadow-soft">
           ...
           <Card className="p-10 bg-[#1E3A8A] text-white border-none rounded-[3rem] shadow-xl shadow-accent/20 relative overflow-hidden group">
